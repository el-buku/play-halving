...
const millisToB = (ts: number) => new anchor.BN(ts).toArrayLike(Buffer, "be", 2)
...
const getUserStateAcc = async (user: PublicKey, programId: PublicKey): Promise<PDADef> => {
    return PublicKey.findProgramAddressSync(
        [sToB(seeds.SEEDS_PREFIX), sToB(seeds.USER_STATE), user.toBuffer()],
        programId
    )
}
const getMillisecondStateAcc = async (timestamp: number, programId: PublicKey): Promise<PDADef> => {
    return PublicKey.findProgramAddressSync(
        [sToB(seeds.SEEDS_PREFIX), sToB(seeds.MILLISECOND_STATE), millisToB(timestamp)],
        programId
    )
}
...
describe("play-halving", () => {
    ...
    const programVault = getAssociatedTokenAddressSync(
        bettingMint, programConfigPDA, true
    )
